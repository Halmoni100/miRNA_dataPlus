...
# save processed data
save(data_proc, file="R_Data/saved_data_proc.r")
...
# account for sequencing depth, so use raw data counts
...
save(uqnorm_data, file="R_Data/saved_uqnorm_data.r")
...
q_norm_data = apply(uqnorm_data, 1, quantile_normalize)
q_norm_data = t(q_norm_data)
# debugging check
print(paste("m =", nrow(q_norm_data)))
print(paste("n =", ncol(q_norm_data)))

# save quantile normalized data
save(q_norm_data, file="R_Data/saved_q_norm_data.r")
...
save(df, feat_df, file="R_Data/data_frames.r")