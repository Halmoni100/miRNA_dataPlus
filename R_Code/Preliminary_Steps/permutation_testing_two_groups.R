...
load("R_Data/saved_q_norm_data.r")
...
column_names = 1:ncol(q_norm_data)
colnames(q_norm_data) = column_names
...
perform_permtest = function(class_list_1, class_list_2) {
	...
	data_matrix = q_norm_data[,samples_used]
	# create designmatrix
...
# Create permutation test results directory
dir.create("Results/permutation_tests")
# Write out summaries to txt files
write.table(v_s_vs_bl_s_result, "Results/permutation_tests/viral_symp_vs_baseline_symp.txt", sep="\t", quote=FALSE)
write.table(v_as_vs_bl_as_result, "Results/permutation_tests/viral_asymp_vs_baseline_asymp.txt", sep="\t", quote=FALSE)
write.table(v_s_vs_v_as_result, "Results/permutation_tests/viral_symp_vs_viral_asymp.txt", sep="\t", quote=FALSE)
write.table(v_s_vs_bc_result, "Results/permutation_tests/viral_symp_vs_bacteria.txt", sep="\t", quote=FALSE)
write.table(bl_s_vs_bl_as_result, "Results/permutation_tests/baseline_symp_vs_baseline_asymp.txt", sep="\t", quote=FALSE)
write.table(bc_vs_bl_result, "Results/permutation_tests/bacteria_vs_baseline.txt", sep="\t", quote=FALSE)
write.table(bl_vs_sick_result, "Results/permutation_tests/baseline_vs_sick.txt", sep="\t", quote=FALSE)
